...
$cat_stmt = $conn->query("SELECT category, COUNT(*) as count FROM items WHERE category != '' GROUP BY category");
$categories = [];
$cat_counts = [];
...
// Top low stock items
$low_stock_stmt = $conn->query("SELECT item_name, quantity FROM items WHERE quantity < 10 ORDER BY quantity ASC LIMIT 5");
$low_stock_names = [];
$low_stock_qtys = [];
while($row = $low_stock_stmt->fetch_assoc()) {
    $low_stock_names[] = $row['item_name'];
    $low_stock_qtys[] = $row['quantity'];
}
...
<main class="main-content">
    ...
    <section class="charts-section" style="margin-bottom: 40px;">
    ...
    <script>
        ...
        new Chart(ctxCat, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: [
                        '#0bbf6a', '#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899'
                    ],
                    borderWidth: 1
                }]
            },
        ...
        new Chart(ctxLow, {
            type: 'bar',
            data: {
                labels: <?php echo json_encode($low_stock_names); ?>,
                datasets: [{
                    label: 'Quantity Remaining',
                    data: <?php echo json_encode($low_stock_qtys); ?>,
        ...
        <?php else: ?>
        document.getElementById('lowStockChart').parentNode.innerHTML = '<p class="text-center" style="padding: 50px; color: #666;">No low stock items!</p>';
        <?php endif; ?>
    </script>
